const PIPELINE_NOMES2 = {
  '86716809': 'Adição - CS'
};

const PIPELINE_IDS2 = ['86716809'];
const SHEET_NAME2 = 'Line Items - Adição CS';
const ETAPA_GANHO_ID2 = '178986534';
let DEALSTAGE_MAP2 = null;

function importarLineItems_AdicaoCS_MesAtual() {
  const hoje = new Date();
  const ano = hoje.getFullYear();
  const mes = hoje.getMonth();
  const inicio = new Date(ano, mes, 1).toISOString();
  const fim = new Date(ano, mes + 1, 0, 23, 59, 59, 999).toISOString();
  importarLineItems_AdicaoCS_porPeriodo(inicio, fim, PIPELINE_IDS2);
}

function importarLineItems_AdicaoCS_porPeriodo(startStr, endStr, pipelineIDs) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(SHEET_NAME2) || ss.insertSheet(SHEET_NAME2);
  ...
  if (!DEALSTAGE_MAP2) {
    DEALSTAGE_MAP2 = obterMapeamentoEtapasPorPipeline_AdicaoCS();
  }
  ...
  for (const pipelineId of pipelineIDs) {
    ...
    do {
      const payload = {
        filterGroups: [{
          filters: [
            ...
            { propertyName: "dealstage", operator: "EQ", value: ETAPA_GANHO_ID2 }
          ]
        }],
      ...
      for (const deal of data.results) {
        ...
        const etapa = formatarNomeEtapa_AdicaoCS(p.dealstage);
        const lineItems = buscarLineItemsDoNegocio_AdicaoCS(deal.id);
        if (!lineItems.length) continue;

        for (const itemId of lineItems) {
          const props = buscarDetalhesDoLineItem_AdicaoCS(itemId);
          if (!props) continue;
          ...
          buffer.push([
            ...
            p.dealname || '',
            p.origem || '',
            PIPELINE_NOMES2[p.pipeline] || p.pipeline || '',
            etapa,
            formatarDataBrasileira_AdicaoCS(p.createdate),
            formatarDataBrasileira_AdicaoCS(p.closedate),
            itemId,
            props.name || '',
            props['f360__tipo_de_produto'] || '',
            props.quantity || '',
            formatarPreco_AdicaoCS(precoLiquido)
          ]);
        }
      }
    } while (after);
...
function obterMapeamentoEtapasPorPipeline_AdicaoCS() {
...
function formatarNomeEtapa_AdicaoCS(dealstageId) {
  return DEALSTAGE_MAP2[dealstageId] || dealstageId;
}

function buscarLineItemsDoNegocio_AdicaoCS(dealId) {
...
function buscarDetalhesDoLineItem_AdicaoCS(id) {
...
function formatarDataBrasileira_AdicaoCS(dataIso) {
  if (!dataIso) return '';
  const d = new Date(dataIso);
  return `${String(d.getDate()).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
}

function formatarPreco_AdicaoCS(valor) {
